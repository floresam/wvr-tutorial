...
class GreatCF extends React.Component {
  ...
  render() {
    return (<div className="greatcf">
      <p>Great! Now try the service with your own data <a className="base--a" href={this.bluemixURL()}>on IBM Cloud</a>.</p>
      <div className="buttonZone">
        <button className="bottomButton" onClick={function() { window.location=this.bluemixURL();}.bind(this)}>Go To IBM Cloud</button>
      </div>
    </div>);
  }
}
...
class ResultsTable extends React.Component {
  render() {
    return (<div className="use--output-data">
          ...
          { this.props.custItems.length ? <CustomClassifyScoreTable category="Classes" rawjson={this.props.custJson} items={this.props.custItems}/> : <div style={{display: 'none'}}></div>}
...
export function classifyScoreTable(results, tagid) {
  let classes = results.images[0].classifiers;
  let tags, food, cust = [];
  let tagsJ, foodJ, custJ = [];
  for(var i=0; i < classes.length; i++){
    let cPath = '/images/0/classifiers/'+i;
    if(classes[i].classifier_id === 'food') {      
      food = jpath.jpath(cPath + '/classes',results,[]);
      foodJ = jpath.jpath(cPath, results);
    } else if (classes[i].classifier_id === 'default') {
      tags = jpath.jpath(cPath + '/classes',results,[]);
      tagsJ =  jpath.jpath(cPath, results) // Json for tags
    } else {
      cust = jpath.jpath(cPath + '/classes',results,[]);
      custJ = jpath.jpath(cPath, results);
    }
  }
  ...
  */
  if (!!foodJ && foodJ.classes[0].class === 'non-food' && foodJ.classes.length === 1) {
    food = [];
    foodJ = [];
  }
